...
# Fixed hyperparameters for model WITHOUT attention
batch_size = 128
beam_size = 5  # Note: we'll use greedy decoding for evaluation
cell_type = 'LSTM'
dec_layers = 3
dropout = 0.2
emb_dim = 32
...
MODEL_PATH = os.path.join(BASE_DIR, 'trained_no_attn_model.pt')
...
# Build model
print(f"Building model WITHOUT attention:")
        ...
        for src, tgt in dataloader:
            ...
            for t in range(1, T):
                logits, dec_h, _ = model.dec(
                    inp, dec_h,
                    None  # No attention context (important!)
                )
...
print("\n" + "="*60)
print("TEST ACCURACY RESULTS (WITHOUT ATTENTION)")
print("="*60)
...
# Save predictions
with open('predictions_without_attention.txt', 'w', encoding='utf-8') as f:
    for pred in test_preds:
        f.write(pred + '\n')

print("Predictions saved to predictions_without_attention.txt")
